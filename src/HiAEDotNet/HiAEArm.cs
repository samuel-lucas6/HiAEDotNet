...
// 'Cycling Index Approach' and 'ARM NEON Optimizations' applied (besides XOR3, which isn't available)
...
internal sealed class HiAEArm : IDisposable
{
    ...
    internal HiAEArm(ReadOnlySpan<byte> key, ReadOnlySpan<byte> nonce)
    {
    ...
    internal void Decrypt(Span<byte> plaintext, ReadOnlySpan<byte> ciphertext, ReadOnlySpan<byte> associatedData = default)
    {
        ...
        if (remainder != 0) {
            ...
            ks[0] = Aes.MixColumns(Aes.Encrypt(_s[(0 + _offset) & HiAE.BlockMask], _s[(1 + _offset) & HiAE.BlockMask])) ^ p[0] ^ _s[(9 + _offset) & HiAE.BlockMask];
    ...
    private void Update(Vector128<byte> xi)
    {
        ...
        var t = Aes.MixColumns(Aes.Encrypt(_s[s0], _s[(1 + _offset) & HiAE.BlockMask])) ^ xi;
        _s[s0] = Aes.MixColumns(Aes.Encrypt(_s[s13], Vector128<byte>.Zero)) ^ t;
    ...
    private void UpdateEnc(ref Vector128<byte> ci, Vector128<byte> mi)
    {
        int s0 = (0 + _offset) & HiAE.BlockMask;
        int s13 = (13 + _offset) & HiAE.BlockMask;
        var t = Aes.MixColumns(Aes.Encrypt(_s[s0], _s[(1 + _offset) & HiAE.BlockMask])) ^ mi;
    ...
    private void UpdateDec(ref Vector128<byte> mi, Vector128<byte> ci)
    {
        ...
        mi = Aes.MixColumns(Aes.Encrypt(_s[s0], _s[(1 + _offset) & HiAE.BlockMask])) ^ t;
        _s[s0] = Aes.MixColumns(Aes.Encrypt(_s[s13], Vector128<byte>.Zero)) ^ t;
        _s[(3 + _offset) & HiAE.BlockMask] ^= mi;
        _s[s13] ^= mi;
        Rol();
